...
class V0AnalyzerHistoHighPtTrack : public edm::EDAnalyzer {
public:
  explicit V0AnalyzerHistoHighPtTrack(const edm::ParameterSet&);
  ~V0AnalyzerHistoHighPtTrack();
...
private:
  ...
  TH1D* InvMass_1_ks[10];
  TH1D* InvMass_1_la[10];

  TH1D* InvMass_1_ks_background[10];
  TH1D* InvMass_1_la_background[10];
...
V0AnalyzerHistoHighPtTrack::V0AnalyzerHistoHighPtTrack(const edm::ParameterSet& iConfig)
...
V0AnalyzerHistoHighPtTrack::~V0AnalyzerHistoHighPtTrack()
{
...
/**
 * Function to calculate the delta R correctly
 * when deltaPhi > 3.14, deltaPhi = 6.28 - deltaPhi;
 * when deltaPhi < -3.14, deltaPhi = 6.28 + deltaPhi;
 * when else, it's deltaPhi;
 */

double deltaR(double raw_deltaPhi, double raw_deltaEta){

  double temp = 0.0;

  if ( raw_deltaPhi > 3.14 ){

    temp = 6.28 - raw_deltaPhi;
  }
  else if( raw_deltaPhi < 3.14 ){
  ...
  r = sqrt( (temp * temp) + (raw_deltaEta * raw_deltaEta) );
...
void
V0AnalyzerHistoHighPtTrack::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace std;
  ...
  for(unsigned it=0; it<v0candidates_ks->size(); ++it){     
   ...
   //Decay length
    ...
    for(unsigned it = 0; it < tracks->size(); it++){
        ...
        double delta_ks_eta = (eta) - (ks_eta);
        double delta_ks_phi = (phi) - (ks_phi);
        double KSconeSize = 0.0;

        KSconeSize = deltaR(delta_ks_phi, delta_ks_eta);
...
/**[fakeTrackLoop]
 * background V0s subtractions( V0s from the medium instead of high pT track or jets)
 * loop over all the fakeTrack, that has opposite sign of eta of the real track, and
 * define a 0.3 coneSize region, find all the V0s inside of the region. Later on they could be 
 * subtracted 
 */
      for (unsigned num = 0; num < fakeTrack.size(); num++){
        ...
        for(unsigned it=0; it<v0candidates_ks->size(); ++it){     
         ...
         //Decay length
          ...
          double delta_fake_ks_eta = fakeTrack[num].eta - (ks_eta);
          double delta_fake_ks_phi = fakeTrack[num].phi - (ks_phi);
          double KSconeSize = 0.0;

          KSconeSize = deltaR(delta_fake_ks_phi, delta_fake_ks_eta);
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[0]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[1]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[2]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                     if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[3]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[4]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[5]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[6]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[7]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[8]->Fill(ks_mass);
                }
                    ...
                    double temp_e = Mass_e(px_dau1,py_dau1,pz_dau1,px_dau2,py_dau2,pz_dau2);
                      if ( temp < 1.13568 && temp > 1.09568 ) continue;
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_1_ks_background[9]->Fill(ks_mass);
                }
            }
          }
        }
...
/**
 * claer the fakeTrack and reuse it on Lambda:
 */
      fakeTrack.clear();
  ...
  for(unsigned it=0; it<v0candidates_la->size(); ++it){
     ...
     //Decay length
      ...
      for(unsigned it = 0; it < tracks->size(); it++){
        ...
        double delta_la_eta = (eta) - (la_eta);
        double delta_la_phi = (phi) - (la_phi);
        double LAconeSize = 0.0;

        LAconeSize = deltaR(delta_la_phi, delta_la_eta);
      ...
      for (unsigned num = 0; num < fakeTrack.size(); num++){
         ...
         for(unsigned it=0; it<v0candidates_la->size(); ++it){
           ...
           //Decay length
            ...
            double delta_fake_la_eta = fakeTrack[num].eta - (la_eta);
            double delta_fake_la_phi = fakeTrack[num].phi - (la_phi);
            double LAconeSize = 0.0;

            LAconeSize = deltaR(delta_fake_la_phi, delta_fake_la_eta);
            ...
            if ( dau1_Nhits > 3 && dau2_Nhits > 3 && dlos > 5 && agl > 0.999 && TMath::Abs(dzos1) > 1 && TMath::Abs(dzos2) > 1 && TMath::Abs(dxyos1) > 1 && TMath::Abs(dxyos2) > 1)
            {
              ...
              { 
                ...
                InvMass_1_la_background[0]->Fill(la_mass);
              }
              ...
              if (la_pt > 1.0 && la_pt < 1.4)
              {
                ...
                InvMass_1_la_background[1]->Fill(la_mass);
              }
              ...
              if (la_pt > 1.4 && la_pt < 1.8)
              {
                ...
                InvMass_1_la_background[2]->Fill(la_mass);
              }
              ...
              if (la_pt > 1.8 && la_pt < 2.2)
              {
                ...
                InvMass_1_la_background[3]->Fill(la_mass);
              }
              ...
              if (la_pt > 2.2 && la_pt < 2.8)
              {
                ...
                InvMass_1_la_background[4]->Fill(la_mass);
              }
              ...
              if (la_pt > 2.8 && la_pt < 3.6)
              {
                ...
                InvMass_1_la_background[5]->Fill(la_mass);
              }
              ...
              if (la_pt > 3.6 && la_pt < 4.6)
              {
                ...
                InvMass_1_la_background[6]->Fill(la_mass);
              }
              ...
              if (la_pt > 4.6 && la_pt < 6.0)
              {
                ...
                InvMass_1_la_background[7]->Fill(la_mass);
              }
              ...
              if (la_pt > 6.0 && la_pt < 9.0)
              {
                ...
                InvMass_1_la_background[8]->Fill(la_mass);
              }
              ...
              if (la_pt > 9.0 && la_pt < 12.0)
              {
                ...
                InvMass_1_la_background[9]->Fill(la_mass);
              }
            }
            }
      }
...
//V0AnalyzerHistoHighPtTrack::analyze{
//if(mult){
  }
}
// ------------ method called once each job just before starting event loop  ------------
void 
V0AnalyzerHistoHighPtTrack::beginJob()
{
  edm::Service<TFileService> fs;
    
  TH1D::SetDefaultSumw2();

  trkPt = fs->make<TH1D>("trkPt", ";pT (GeV/c);#Events", 10000,0,30 );

    for(int i = 0; i<10; i++)
     { 

       InvMass_1_ks[i] = fs->make<TH1D>(Form("InvMass_1_ks%d",i),";mass(GeV/c^{2});#Events",360,0.44,0.56);
       InvMass_1_ks_background[i] = fs->make<TH1D>(Form("InvMass_1_ks_background%d",i),";mass(GeV/c^{2});#Events",360,0.44,0.56);
       
       InvMass_1_la[i] = fs->make<TH1D>(Form("InvMass_1_la%d",i),";mass(GeV/c^{2});#Events",360,1.08,1.16);
       InvMass_1_la_background[i] = fs->make<TH1D>(Form("InvMass_1_la_background%d",i),";mass(GeV/c^{2});#Events",360,1.08,1.16);
...
void 
V0AnalyzerHistoHighPtTrack::endJob() {
}

//define this as a plug-in
DEFINE_FWK_MODULE(V0AnalyzerHistoHighPtTrack);