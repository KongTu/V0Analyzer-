...
class V0AnalyzerHisto_Dmesson : public edm::EDAnalyzer {
public:
  explicit V0AnalyzerHisto_Dmesson(const edm::ParameterSet&);
  ~V0AnalyzerHisto_Dmesson();
...
V0AnalyzerHisto_Dmesson::V0AnalyzerHisto_Dmesson(const edm::ParameterSet& iConfig)
 
{
  ...
  doPrimaryTrack_ = iConfig.getUntrackedParameter<bool>("doPrimaryTrack", false);
  
}


V0AnalyzerHisto_Dmesson::~V0AnalyzerHisto_Dmesson()
{
...
double Mass_D(double px_1,double py_1,double pz_1,double px_2,double py_2,double pz_2, double kmass ){

  double temp = 0.0;
        double E1 = sqrt((px_1*px_1+py_1*py_1+pz_1*pz_1)+(0.13957*0.13957));
        double E2 = sqrt((px_2*px_2+py_2*py_2+pz_2*pz_2)+(kmass*kmass));
        double E_tot = E1+E2;
...
void
V0AnalyzerHisto_Dmesson::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
                    ...
                    if ( temp < 1.13568 && temp > 1.09568 ) continue;
                      if ( temp_e < 0.015511) continue;
                        ...
                        InvMass_ks[0]->Fill(ks_mass);
                          ...
                          if(ks_mass < 0.5076 && ks_mass > 0.4876){

                            double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                              InvMass_d[0]->Fill( temp_dMass );
                            double temp_pionMass = temp_dMass - ks_mass;
                              InvMass_pion[0]->Fill( temp_pionMass );
                      ...
                      if ( temp_e < 0.015511) continue;
                        ...
                        InvMass_ks[1]->Fill(ks_mass);
                          ...
                          if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[1]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[1]->Fill( temp_pionMass );
                      ...
                      if ( temp_e < 0.015511) continue;
                        ...
                        InvMass_ks[2]->Fill(ks_mass);
                          ...
                          if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[2]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[2]->Fill( temp_pionMass );
                      ...
                      if ( temp_e < 0.015511) continue;
                        ...
                        InvMass_ks[3]->Fill(ks_mass);
                          ...
                          if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[3]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[3]->Fill( temp_pionMass );
                      ...
                      if ( temp_e < 0.015511) continue;
                        ...
                        InvMass_ks[4]->Fill(ks_mass);
                          ...
                          if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[4]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[4]->Fill( temp_pionMass );
                        ...
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_ks[5]->Fill(ks_mass);
                            ...
                            if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[5]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[5]->Fill( temp_pionMass );
                        ...
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_ks[6]->Fill(ks_mass);
                            ...
                            if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[6]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[6]->Fill( temp_pionMass );
                        ...
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_ks[7]->Fill(ks_mass);
                            ...
                            if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[7]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[7]->Fill( temp_pionMass );
                        ...
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_ks[8]->Fill(ks_mass);
                            ...
                            if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[8]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[8]->Fill( temp_pionMass );
                        ...
                        if ( temp_e < 0.015511) continue;
                          ...
                          InvMass_ks[9]->Fill(ks_mass);
                            ...
                            if(ks_mass < 0.5076 && ks_mass > 0.4876){
                              ...
                              double temp_dMass = Mass_D(px,py,pz,ks_px,ks_py,ks_pz,ks_mass);
                                InvMass_d[9]->Fill( temp_dMass );
                              double temp_pionMass = temp_dMass - ks_mass;
                                InvMass_pion[9]->Fill( temp_pionMass );
...
void 
V0AnalyzerHisto_Dmesson::beginJob()
{
  ...
  TH1D::SetDefaultSumw2();
    ...
    for(int i = 0; i<10; i++)
     {
       InvMass_ks_nocut[i] = fs->make<TH1D>(Form("InvMass_ks_nocut%d",i),";mass(GeV/c^{2});#Events",360,0.44,0.56);
       InvMass_ks[i] = fs->make<TH1D>(Form("InvMass_ks%d",i),";mass(GeV/c^{2});#Events",360,0.44,0.56);
       InvMass_la_nocut[i] = fs->make<TH1D>(Form("InvMass_la_nocut%d",i),";mass(GeV/c^{2});#Events",360,1.08,1.16);
       InvMass_la[i] = fs->make<TH1D>(Form("InvMass_la%d",i),";mass(GeV/c^{2});#Events",360,1.08,1.16);
       InvMass_d[i] = fs->make<TH1D>(Form("InvMass_d%d",i),";mass(GeV/c^{2});#Events",720,1.5,2.2);
       InvMass_pion[i] = fs->make<TH1D>(Form("InvMass_pion%d",i),";mass(GeV/c^{2});#Events",240,0.11,0.17);
     }
}

// ------------ method called once each job just after ending the event loop  ------------
void 
V0AnalyzerHisto_Dmesson::endJob() {
}

//define this as a plug-in
DEFINE_FWK_MODULE(V0AnalyzerHisto_Dmesson);